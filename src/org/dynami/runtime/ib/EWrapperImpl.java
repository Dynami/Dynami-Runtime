...
public class EWrapperImpl implements EWrapper {
	...
	//! [socket_init]
	public EWrapperImpl(IBService service) {
		readerSignal = new EJavaSignal();
		clientSocket = new EClientSocket(this, readerSignal);

		msg = org.dynami.runtime.impl.Execution.Manager.msg();
	...
	public void tickPrice(int tickerId, int field, double price, TickAttrib attribs) {
		...
		if(Utils.in(TickType.get(field), TickType.ASK, TickType.DELAYED_ASK)){
			msg.async(
					Topics.ASK_ORDERS_BOOK_PREFIX.topic,
					new Book.Orders(asset.symbol, DTime.Clock.getTime(), Book.Side.ASK, 1, price, Book.NONE)
			);
		}
		if(Utils.in(TickType.get(field), TickType.BID, TickType.DELAYED_BID)){
			msg.async(
					Topics.ASK_ORDERS_BOOK_PREFIX.topic,
					new Book.Orders(asset.symbol, DTime.Clock.getTime(), Book.Side.BID, 1, price, Book.NONE)
			);
		}
	}
	//! [tickprice]
	...
	public void tickSize(int tickerId, int field, Decimal size) {
		...
		if(Utils.in(TickType.get(field), TickType.ASK_SIZE, TickType.DELAYED_ASK_SIZE)){
			msg.async(
					Topics.ASK_ORDERS_BOOK_PREFIX.topic+asset.symbol,
					new Book.Orders(asset.symbol, DTime.Clock.getTime(), Book.Side.ASK, 1, Book.NONE, size.longValue())
			);
		}
		if(Utils.in(TickType.get(field), TickType.BID_SIZE, TickType.DELAYED_BID_SIZE)){
			msg.async(
					Topics.BID_ORDERS_BOOK_PREFIX.topic+asset.symbol,
					new Book.Orders(asset.symbol, DTime.Clock.getTime(), Book.Side.BID, 1, Book.NONE, size.longValue())
			);
		}
	}
	//! [ticksize]
   ...
   @Override
    ...
    public void tickByTickBidAsk(int reqId, long time, double bidPrice, double askPrice, Decimal bidSize, Decimal askSize, TickAttribBidAsk tickAttribBidAsk) {
		...
		Asset.Tradable asset = service.getAsset(reqId);

		msg.async(
			Topics.ASK_ORDERS_BOOK_PREFIX.topic+asset.symbol,
			new Book.Orders(asset.symbol, time, Book.Side.ASK, 1, askPrice, askSize.longValue())
		);
		msg.async(
			Topics.BID_ORDERS_BOOK_PREFIX.topic+asset.symbol,
			new Book.Orders(asset.symbol, time, Book.Side.BID, 1, bidPrice, bidSize.longValue())
		);
	}
    //! [tickbytickbidask]
